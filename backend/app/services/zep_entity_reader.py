    ...
    def filter_defined_entities(
        ...
        for node in all_nodes:
            ...
            if not custom_labels:
                # Zep Cloud can return valid graph nodes without labels when
                # custom ontology labels are not materialized by the graph
                # backend. Keep those real nodes as generic entities instead
                # of silently producing an empty simulation population. We do
                # not infer a domain type; profile generation uses Entity.
            ...
            # 如果指定了预定义类型，检查是否匹配
            if custom_labels and defined_entity_types:
