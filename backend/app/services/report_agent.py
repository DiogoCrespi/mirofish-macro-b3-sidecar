...
class ReportAgent:
    ...
    def plan_outline(
        ...
        try:
            ...
            # MacroB3's structured-report contract needs a bounded, auditable
            # report.  The sidecar's generic outline model may return five
            # sections, which makes the CPU/Ollama ReACT pass exceed the
            # caller's deadline.  Keep the titles generated by the model but
            # cap this contract at two sections; no scenario content is
            # synthesized here.
            if "5A.3-mirofish-scenario-report-v1" in self.simulation_requirement:
                sections = sections[:2]
